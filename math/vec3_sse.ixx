...
namespace deckard::math::sse
{
	...
	export struct alignas(16) vec3
	{
		...
		bool is_close_enough(const vec3& lhs, float epsilon = 0.0000001f) const noexcept
		{
			...
			auto diff        = masked_this - (lhs * vec3(xyzmask));
	...
	export [[nodiscard("Use the divided value")]] vec3 safe_divide(const vec3& lhs, const vec3& rhs) { return lhs / rhs; }

	export [[nodiscard("Use the divided value")]] vec3 safe_divide(const vec3& lhs, const float scalar) { return lhs / scalar; }
