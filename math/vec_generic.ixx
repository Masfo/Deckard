...
namespace deckard::math
{
	...
	struct generic_vec2
	{
		...
		constexpr generic_vec2(const std::initializer_list<T> list, const std::source_location& loc = std::source_location::current())
		{
			if (list.size() >= 2)
			{
				x = list[0];
		...
		constexpr bool has_zero() const { return x == 0 or y == 0; };

		constexpr bool is_zero() const { return x == 0 or y == 0; };
		...
		constexpr generic_vec2& operator++()
		{
			*this += generic_vec2(1);
			return *this;
		}

		constexpr generic_vec2 operator++(int)
		{
			generic_vec2 tmp = *this;
			*this += generic_vec2(1);
			return tmp;
		}
		...
		constexpr void operator+=(const generic_vec2& other)
		{
			x += other.x;
			y += other.y;
		}

		constexpr generic_vec2 operator+(const generic_vec2& other) const
		{
			generic_vec2 result = *this;
			result += other;
			return result;
		}
		...
		constexpr generic_vec2 operator/(const U& scalar) const
		{
			if(math::is_close_enough_zero(scalar))
		...
		constexpr generic_vec2& operator-()
		{
			*this *= generic_vec2(T{-1});

			return *this;
		}

		constexpr generic_vec2 operator+() const { return *this; }
		...
		constexpr auto operator<=>(const generic_vec2& other) const = default;

		constexpr bool operator==(const generic_vec2& other) const { return equals(other, T{}); }

		constexpr bool equals(const generic_vec2& other, const T epsilon = T(0.000001)) const
		requires(std::floating_point<T>)
		{
			return is_close_enough(other, epsilon);
		}

		constexpr bool equals(const generic_vec2& other, const T epsilon = 0) const
		requires(std::integral<T>)
		{
			return is_close_enough(other, epsilon);
		}

		constexpr bool is_close_enough(const generic_vec2& lhs, T epsilon = T{0.000001} ) const
		requires(std::floating_point<T>)
		{
		...
		constexpr bool is_close_enough(const generic_vec2& lhs, T epsilon = 0) const
		requires(std::integral<T>)
		{
			if (not math::is_close_enough(x, lhs.x, epsilon))
				return false;
		...
		static inline generic_vec2 zero() { return generic_vec2(T{0}); }

		static inline generic_vec2 one() { return generic_vec2(T{1}); }
