...
TEST_CASE("bigint", "[bigint]")
{
	...
	SECTION("multiply")
	{
		...
		bigint p("6413528947707158027879019017057738908482501474294344720811685963202453234463023862359875266834770873766192"
				 "5585694639798853367");

		bigint q("3337202759497815655622601060535511422794076034476755466678452098702384172921003708025744867329688187756571"
				 "8986258036932062711");

		bigint correct(
		  "2140324650240744961264423072839333563008614715144755017797754920881418023447140136643345519095804679"
		  "6109928518724709145876873962"
		  "6192155736304745477052080511905649310668769159001975940569345745223058932597669747168173806936489469"
		...
		CHECK(rsa250_double.to_string() == "12827057895414316055758038034115477816965002948588689441623371926404906468926047"
										   "7247197505336695417475323851171389279597706734");


		bigint rsa_a_neg = p * -1;
		CHECK(rsa_a_neg.signum() == Sign::negative);
		CHECK(rsa_a_neg.to_string() == "-64135289477071580278790190170577389084825014742943447208116859632024532344630238623"
									   "598752668347708737661925585694639798853367");

		rsa_a_neg *= -1;
		CHECK(rsa_a_neg.signum() == Sign::positive);
		CHECK(rsa_a_neg.to_string() == "641352894770715802787901901705773890848250147429434472081168596320245323446302386235"
	...
	SECTION("divide")
	{
		...
		CHECK(p.to_string() == "32067644738535790139395095085288694542412507371471723604058429816012266172315119311799376334"
							   "173854368830962792847319899426683");
	}
	...
	SECTION("pow")
	{
		...
		CHECK(c.to_string() ==
			  "3229001753263951338868893625587348535314611763285183215485221668591463283958973555956598356990146774555994487"
			  "5449797671314352532"
			  "1251777580415394253890472769231524615134545021811369931100771306900461933058834613272332134738367268724362824"
			  "6035531829091587859"
			  "6939195157955905767130134752581191460596795355261275651729819196521697845745019711260824560639905752076567808"
			  "095289");
		CHECK(c.signum() == Sign::positive);
		CHECK(c.count() == 371);
		...
		CHECK(aaa.to_string() ==
			  "-36560683182918408208763683029278388669727354257481133094004576420896340840189961852818259"
			  "6556243961192043721542111882547153"
			  "579244270460323674958722095560806595915245100899962245232488846537273106084009466343331951"
			  "065920293331146240234375");
		CHECK(aaa.signum() == Sign::negative);
	}
	...
	SECTION("gcd")
	{
		...
		CHECK(b.to_string() ==
			  "32733906078961418700131896968275991522166420460430647894832913680961337964046745548832700923259041571508866"
			  "84127560071009217256545885393053328527589376");

		result = gcd(a, b);
		CHECK(result.to_string() ==
			  "327339060789614187001318969682759915221664204604306478948329136809613379640467455488327009232590415715"
	...
	SECTION("bitwise not")
	{
		...
		a =
		  "0b10001110000110001011010010010110010100100110100110111001110011100111011100101101110000001001001011011110101011";
		b = ~a;
		CHECK(
		  b.to_string(2) ==
	...
	SECTION("bitwise xor")
	{
		...
		CHECK(a.to_string(2) ==
			  "1111000111110100110100111000011100010010011001110000001100010010101000101111010001000110001011010000110001"
			  "1001100110111010111010000101000011000110");
		CHECK(b.to_string(2) ==
			  "1000100101110100000010100110001011100110111100010011011010001000010110000001100100111011001010010101111111"
			  "0010011101111110001100101110110001010110");
		CHECK(c.to_string(2) ==
			  "1111000100000001101100111100101111101001001011000110101100110101111101011101101011111010000010001010011101"
