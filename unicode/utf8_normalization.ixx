...
namespace deckard::utf8
{
	...
	namespace normalization
	{
		...
		void canonical_sort(std::vector<char32>& codepoints)
		{
			...
			while (it != end)
			{
				// 1. Skip past any starter characters (CCC == 0)
				while (it != end and get_ccc(*it) == 0)
				{
					++it;
				}

				// 2. Find the end of the current non-starter block
				auto block_end = it;
				while (block_end != end and get_ccc(*block_end) != 0)
				{
					++block_end;
				}

				// 3. Sort only this specific contiguous block of combining marks
		...
		[[nodiscard]] std::vector<char32> compose_codepoints(std::vector<char32> nfd)
		{
			...
			for (size_t i = 1; i < nfd.size(); ++i)
			{
				...
				else
				{
					...
					{
						result.push_back(C);
						// Only update max_ccc if the mark was actually saved as a separate trailing element
